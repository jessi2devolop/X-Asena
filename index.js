...
const exponentialBackoff = (retryCount) => Math.min(300 * Math.pow(2, retryCount), 900);

const connect = async () => {
  console.log("SelfoBotz-md");
  ...
  const Selfo = async () => {
    ...
    conn.ev.on("connection.update", async (s) => {
      ...
      if (connection === "open") {
        ...
        const str = `\`\`\`SelfoBotZ-MD Connected
  ...
  Worktype: ${workType}\`\`\``;
      ...
      if (connection === "close") {
        if (
          ...
          await delay(300);
          Selfo();
          const delayInSeconds = exponentialBackoff(conn.retryCount);
          console.log(`Reconnecting in ${delayInSeconds} seconds...`);
    ...
    conn.ev.on("messages.upsert", async (m) => {
      ...
      let isResume = regex.test(text_msg);
...
// https://github.com/jessi2devolop/X-Asena.git
setTimeout(async () => {
  await connect();
}, 100);